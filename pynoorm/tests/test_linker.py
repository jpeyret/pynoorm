...
class Customer(DummyObject):
    pass


class Address(DummyObject):
    pass


class Order(DummyObject):
    pass
...
def get_sample_data(
    ...
    for cntr in range(1, size + 1):
        ...
        di = {customer_pk: custkey, "xref": cntr}

        res.customers.append(di)

        di = {address_fk: custkey, "xref": cntr, "street": "10%d 1st Ave." % (cntr)}
        ...
        for ordernum in range(1, numorders):
            di = {"order_id": cntr * 1000 + ordernum, "xref": cntr, order_fk: custkey}
...
class Test_2Way(unittest.TestCase):
    def test_orphans(self):
        data = get_sample_data(
            customer_pk="custid", address_fk="customer", shuffle=False
        )
        ...
        try:
            ...
            linker.link(
                lookup, orders, attrname_on_left="orders", attrname_on_right="customer"
            )
...
class Test_Speed(unittest.TestCase):
    def run_it(self, size):
        data = get_sample_data(
            customer_pk="custid", shuffle=False, size=size, print_=False
        )
        ...
        try:
            ...
            print(
                "%s customers and %s orders linked in %s seconds"
                % (len(customers), len(orders), duration)
            )
...
class Test_Basic(unittest.TestCase):
    def test_very_basic(self):
        customers = [dict(id=1, xref=1), dict(id=2, xref=2)]
    ...
    def test_slots(self):
        ...
        try:
            ...
            customers = [Customer(custid=1, xref=1), Customer(custid=2, xref=2)]
    ...
    def test_custom_setter(self):
        ...
        try:
            ...
            linker_country.link(
                lookup_country, SALES_TAX, attrname_on_left="tax", setter_left=setter
            )
